...
| **Tier 3** | > 2000 lines (> 100K words) | Pre-split + parallel + merge — handled by separate scripts (`agentic-extraction/chunk_large_docs.py`, `agentic-extraction/merge_chunk_results.py`). When you receive a chunk from those scripts, treat it as a Tier 1 input. |
