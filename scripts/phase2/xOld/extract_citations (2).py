...
- VERSION 3.3: Added document_id tracking in Citation records
"""
...
<definitions>
1. **Foreign Citation**: A citation to a court in a DIFFERENT country (not {source_country}).
   Example: A U.S. court citing a Canadian Supreme Court case.

2. **International Citation**: A citation to an INTERNATIONAL court that has BINDING jurisdiction over {source_country}.
   The binding courts for {source_country} are: {binding_courts_str}.
   These courts' decisions are binding or persuasive for {source_country}.

3. **Foreign International Citation**: A citation to an INTERNATIONAL court that does NOT have binding jurisdiction over {source_country}.
...
def process_single_document(doc, session, stats):
    ...
    INPUT:
        - doc: Document object with text, metadata, and IDs
        - session: SQLAlchemy session
        - stats: Statistics dictionary
    
    ALGORITHM:
        1. Classify document using Sonnet (is it a judicial decision?)
        2. If decision, extract citations using Haiku
        3. Store citations WITH document_id tracking
    ...
    try:
        ...
        # VERSION 3.3: Added document_id to Citation records
        for i, c in enumerate(valid_citations):
            ...
            cit_record = Citation(
                ...
                case_id=doc.case_id,
                document_id=doc.document_id,  # NEW: Track source document
