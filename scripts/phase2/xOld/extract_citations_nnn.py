...
=====================================================================
VERSION 2.0 - FULL TEXT PROCESSING (No Truncation for Citation Extraction)

STEP 1: Verifies if document is a judicial decision using Claude API
        (Uses first 3000 chars only - sufficient for classification)
        
STEP 2: If YES → Extracts FOREIGN and INTERNATIONAL citations from FULL TEXT
...
Key Changes in v2.0:
- ✅ Removed text truncation for citation extraction (previously 100k char limit)
- ✅ Full document text now processed to ensure no citations are missed
- ✅ Added document length monitoring and logging
- ✅ Safety check for extreme outliers approaching model token limits
- ✅ Classification still uses truncated text (3000 chars sufficient)

Features:
- Document Type Validation: Verifies document is actually a judicial decision
- Full Text Processing: Analyzes entire document for comprehensive citation capture
...
- Length Monitoring: Tracks and logs document sizes for analysis
"""
...
def check_if_judicial_decision(document_text, metadata):
    ...
    INPUT:
    - document_text: Full text of the document (truncated to first 3000 chars for classification)
    ...
    2. Sends first 3000 characters to Claude API with classification criteria
       (Full text not needed - doc type evident from beginning)
    ...
    NOTE: Classification uses truncated text (3000 chars) as this is sufficient
          to determine document type. Citation extraction uses FULL text.
    """
    ...
    # Construct classification prompt (using truncated text - sufficient for classification)
...
DOCUMENT TEXT (First {classification_text_limit} characters):
{document_text[:classification_text_limit]}

...

[Text truncated for classification purposes - first {classification_text_limit} chars sufficient]
"""
...
def find_citation_indices(full_text, citation_string_raw):
    ...
    - citation_string_raw: Exact citation string to locate
    
    ALGORITHM:
    - Performs simple first-match search in full text
    - Returns character indices for highlighting
    
    OUTPUT:
    - (start_index, end_index) tuple, or (None, None) if not found
    """
...
def estimate_token_count(text):
    ...
    - Accounts for multilingual content (Portuguese/Spanish use ~4.5 chars/token)
    ...
    if not text:
        return 0
    # Conservative estimate: 4 chars per token (safe for multilingual content)
    return len(text) // 4
...
def generate_citation_extraction_prompt(document_text, source_court, source_country, source_jurisdiction):
    ...
    VERSION 2.0 CHANGE: NO TEXT TRUNCATION
    - Processes FULL document text to ensure comprehensive citation capture
    - Removed previous 100k character limit
    - Only safety check: warns if approaching model token limits (rare)
    
    INPUT:
    - document_text: FULL extracted text from judicial decision (no truncation)
    - source_court: Name of court issuing the decision
    - source_country: Country of source court
    - source_jurisdiction: Specific jurisdiction details
    
    ALGORITHM:
    1. Estimates token count for safety monitoring
    2. Logs warning if document is extremely long (approaching model limits)
    3. Constructs prompt with full text and negative constraints
    4. Returns complete prompt for LLM processing
    
    OUTPUT:
    - Formatted prompt string with full document text included
    """
    
    # Monitor document length for cost tracking and safety
    doc_length_chars = len(document_text)
    doc_length_tokens_approx = estimate_token_count(document_text)
    
    # Log document size
    logging.info(
        f"Document length: {doc_length_chars:,} chars "
        f"(~{doc_length_tokens_approx:,} tokens estimated)"
    )
    
    # Safety check: Warn if approaching model limits (extremely rare)
    safe_threshold = CONFIG['SAFE_TOKEN_THRESHOLD']
    if doc_length_tokens_approx > safe_threshold:
        logging.warning(
            f"⚠️  LARGE DOCUMENT: ~{doc_length_tokens_approx:,} tokens. "
            f"Approaching model limit ({CONFIG['MODEL_CONTEXT_WINDOW']:,} tokens). "
            f"Processing full text anyway - model will handle truncation if needed."
        )
    
    # NO TRUNCATION - Process full text to ensure no citations are missed
    # Old code (removed):
...
def process_single_document(doc, session, stats):
    ...
    try:
        ...
        logging.info(f"\n{'='*70}")
        logging.info(f"Processing Document ID: {doc.document_id}")
        logging.info(f"Checking if document is a judicial decision...")
        ...
        # ========================================================================
        # STEP 2: CITATION EXTRACTION (Full Text Processing)
        # ========================================================================
        ...
        logging.info(f"Extracting citations from FULL document text...")
        
        # Generate prompt with FULL text (no truncation)
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    raise e
                logging.warning(f"Retry attempt {attempt + 1}/3 after error: {e}")
                time.sleep(2)
        ...
        logging.info(
            f"Found {len(valid_citations)} foreign citations "
            f"(excluded {data.get('domestic_citations_excluded', 0)} domestic). "
...
def main():
    ...
    logging.info("VERSION 2.0 - FULL TEXT PROCESSING (No Truncation)")
    logging.info("Climate Litigation Database Analysis")
    logging.info("="*70)
    logging.info(f"Model: {CONFIG['ANTHROPIC_MODEL']}")
    logging.info(f"Processing: FULL document text (no length limits)")
    logging.info(f"Min Confidence: {CONFIG['MIN_CONFIDENCE']}")
    logging.info(f"Classification: First {CONFIG['CLASSIFICATION_TEXT_LIMIT']} chars only")
    logging.info("="*70)
    ...
    try:
        ...
        logging.info("="*70)
        logging.info("\n✅ Full text processing ensures comprehensive citation capture")
        logging.info("   No citations missed due to document length truncation")
        logging.info("="*70)
