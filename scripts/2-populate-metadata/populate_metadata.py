...
# Import config
from config import DATABASE_FILE, LOGS_DIR, TRIAL_BATCH_CONFIG, UUID_NAMESPACE
from gcp_secrets import get_engine
from test_run import add_test_run_arg, sample_dataframe

# Import database models
sys.path.insert(0, os.path.join(_SCRIPTS_DIR, "0-initialize-database"))
from uuid import uuid5

from init_database import Case, Document
...
def parse_jurisdiction(jurisdiction_str):
    ...
    if len(parts) >= 2:
        second_part = parts[1]
        if any(
            indicator in second_part for indicator in ["Ct.", "Court", "Tribunal", "Commission"]
        ):
            return second_part
...
def create_metadata_json(row, metadata_type="case"):
    ...
    if metadata_type == "case":
        ...
        # List fields
        list_mappings = {"languages": "Language(s)", "bundle_names": "Bundle Name(s)"}
        for key, col in list_mappings.items():
            if pd.notna(row.get(col)):
                metadata[key] = [x.strip() for x in str(row[col]).split(";") if x.strip()]
...
def process_case(row, session):
    ...
    parse_country_from_geographies(row.get("Geographies"))
    region = parse_region(row.get("Geography ISOs"))
    filing_date, decision_date = extract_timeline_dates(
        row.get("First event in timeline"), row.get("Last event in timeline")
    )
    ...
    case_data = {
        ...
        else None,  # FIX: Populate geographies column
        "region": region,
        ...
        "sabin_case_id": str(row["Case ID"]).strip() if pd.notna(row.get("Case ID")) else None,
        "sabin_internal_case_id": str(row["Internal Case ID"]).strip()
...
def process_document(row, case, session):
    ...
    doc_data = {
        ...
        "document_type": str(row["Document Type"])
        if pd.notna(row.get("Document Type"))
        else "Decision",
        "document_url": str(row["Document Content URL"])
        if pd.notna(row.get("Document Content URL"))
        else None,
        # Sabin Center identifiers (D4)
        "sabin_document_id": str(row["Document ID"]).strip()
        if pd.notna(row.get("Document ID"))
        else None,
        "sabin_internal_document_id": str(row["Internal Document ID"]).strip()
...
def populate_database(test_run=None, seed=42):
    stats = {
        "total_rows": 0,
        "cases_created": 0,
        "cases_updated": 0,
    ...
    # Apply test-run sampling: sample N document rows, then expand by Case ID
    if test_run is not None:
        sampled_docs = sample_dataframe(df, test_run, seed)
        sampled_case_ids = sampled_docs["Case ID"].unique()
        df = df[df["Case ID"].isin(sampled_case_ids)].copy()
        logging.info(
            f"Test-run: {test_run} documents sampled → "
            f"{len(sampled_case_ids)} cases, {len(df)} total rows (expanded by Case ID)"
        )
    ...
    logging.info("\n" + "=" * 70)
    logging.info("POPULATION SUMMARY")
    logging.info("=" * 70)
    ...
    if TRIAL_BATCH_CONFIG["ENABLED"]:
        ...
        logging.info(
            f"  Processed {filter_stats['filtered']} out of {filter_stats['original']} total documents"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Populate database metadata from Excel")
    add_test_run_arg(parser)
    args = parser.parse_args()

    populate_database(test_run=args.test_run, seed=args.seed)
