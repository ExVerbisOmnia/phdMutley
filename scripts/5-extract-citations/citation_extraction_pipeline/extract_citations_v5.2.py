#!/usr/bin/env python3
"""
Citation Extraction Script - Version 5.1 (Source Jurisdiction Fix)
======================================================================
...
VERSION 5.1 - SOURCE JURISDICTION FIX
=====================================

CHANGES FROM v5.0:
- Fixed source_jurisdiction population by fetching Case.geographies directly
- Added KNOWN_COUNTRIES dictionary with 100+ countries
- New extract_country_from_geographies() function with smart parsing
- Query now fetches Case.geographies instead of relying on metadata_data
- Handles both "Country;State" and "State;Country" format variations
...
# ============================================================================
# INPUT: None
# PURPOSE: Master list of countries for extracting country from Case.geographies
#          Handles both "Country;State" and "State;Country" formats
# OUTPUT: Set of country names (used by extract_country_from_geographies)
...
KNOWN_COUNTRIES = {
    ...
    "Latvia",
    "Lithuania",
    # EUROPE - SOUTHEASTERN & MICRO-STATES
    "Greece",
    "Turkey",
    "Türkiye",
    "Cyprus",
...
def extract_context_sentences(
    text: str, start_index: int, end_index: int, num_sentences: int = 3
) -> tuple[str, str]:
    """
...
def get_source_region(country: str) -> str:
    ...
    # Note: get_binding_courts() only takes 1 argument (country)
    get_binding_courts(country)
...
def extract_country_from_geographies(geographies_string: str) -> str:
    ...
    INPUT: Geography string from Case.geographies column, e.g.:
        - "United States;New York" (country first)
        - "Ontario;Canada" (state first)
        - "Brazil" (single value)
        - "Pará;Brazil" (state first, Brazilian case)

    ALGORITHM:
        1. If empty/null, return "Unknown"
        2. Split by semicolon and strip whitespace
        3. Check each part against KNOWN_COUNTRIES set
        4. Return first country found
        5. If no country found, return first part as fallback

    OUTPUT: Country name string (e.g., "United States", "Brazil", "Canada")
    """
    if not geographies_string:
        return "Unknown"
    ...
    # Return first part as fallback (might be unrecognized country)
...
def generate_phase2_extraction_prompt(
...
def extract_all_case_references_phase2(
    document_id: uuid.UUID, raw_text: str, source_jurisdiction: str, source_region: str, session
) -> dict | None:
    """
    ...
    try:
        ...
        data["phase_2_tokens_input"] = result["tokens_in"]
        data["phase_2_tokens_output"] = result["tokens_out"]
        data["phase_2_model"] = result["model"]
...
def identify_origin_tier2_sonnet(
    case_name: str, raw_text: str, context_before: str, context_after: str
) -> dict | None:
    """
...
def process_single_document_phased(doc_tuple, session, stats: dict) -> bool:
    ...
    """
    # Unpack query results (v5.1: now includes geographies from Case table)
    document_id = doc_tuple[0]
    metadata_data = doc_tuple[1]
    raw_text = doc_tuple[2]
    case_id = doc_tuple[3]
    geographies = doc_tuple[4]  # v5.1: fetched directly from Case.geographies
    ...
    try:
        ...
        # v5.1 FIX: Fallback to metadata_data if Case.geographies is NULL
        # This handles cases where geography data wasn't imported into Case table
        ...
        if len(references) == 0:
            ...
            summary = CitationExtractionPhasedSummary(
                ...
                total_cost_usd=(total_tokens_input / 1e6 * 0.25)
                + (total_tokens_output / 1e6 * 1.25),
        ...
        for i, ref in enumerate(references):
            ...
            origin_data = identify_case_origin(
                ref.get("case_name", ""), ref.get("raw_text", ""), context_before, context_after
            )
    ...
    try:
        ...
        query = (
            session.query(
                ...
                Case.case_id,
                Case.geographies,  # Added in v5.1: fetch geography directly from Case table
            )
            .join(ExtractedText, Document.document_id == ExtractedText.document_id)
            .join(Case, Document.case_id == Case.case_id)
            .filter(
                ExtractedText.raw_text is not None,
                Document.is_decision,  # Only process decisions
            )
        )
        ...
        processed_ids = [
            id[0] for id in session.query(CitationExtractionPhasedSummary.document_id).all()
        ]
        if processed_ids:
