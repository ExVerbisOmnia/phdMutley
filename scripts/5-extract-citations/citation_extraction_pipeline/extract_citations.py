...
class CitationExtractionPhasedSummary(Base):
    ...
    document_id = Column(pgUUID(as_uuid=True), ForeignKey('documents.document_id'), nullable=False, unique=True)
...
def process_single_document_phased(doc_tuple, session, stats: Dict) -> bool:
    ...
    try:
        ...
        geographies = metadata_json.get('Geographies', '') if isinstance(metadata_json, dict) else ''
