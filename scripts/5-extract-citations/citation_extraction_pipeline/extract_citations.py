...
class CitationExtractionPhased(Base):
    """
    Model for phased citation extraction results.
    Stores individual citations with full phase tracking.
    """

    __tablename__ = "citation_extraction_phased"

    extraction_id = Column(pgUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    document_id = Column(pgUUID(as_uuid=True), ForeignKey("documents.document_id"), nullable=False)
    case_id = Column(pgUUID(as_uuid=True), ForeignKey("cases.case_id"))
    ...
    citation_format = Column(String(100))
...
class CitationExtractionPhasedSummary(Base):
    """
    Model for document-level extraction summary.
    Stores aggregate metrics per document.
    """

    __tablename__ = "citation_extraction_phased_summary"

    summary_id = Column(pgUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    document_id = Column(
        pgUUID(as_uuid=True), ForeignKey("documents.document_id"), nullable=False, unique=True
    )
...
def get_source_region(country: str) -> str:
    ...
    get_binding_courts(country, "Unknown")
...
def extract_all_case_references_phase2(
    ...
    try:
        ...
        data["phase_2_model"] = "claude-haiku-4-20250514"
...
def process_single_document_phased(doc_tuple, session, stats: dict) -> bool:
    ...
    try:
        ...
        geographies = (
            metadata_json.get("Geographies", "") if isinstance(metadata_json, dict) else ""
        )
        ...
        for i, ref in enumerate(references):
            ...
            citation_record = CitationExtractionPhased(
                ...
                # Processing metadata
                phase_2_model="claude-haiku-4-20250514",
