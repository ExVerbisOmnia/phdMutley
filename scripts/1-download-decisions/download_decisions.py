...
from config import CONFIG, DATABASE_FILE, LOGS_DIR, PDF_DOWNLOAD_DIR, TRIAL_BATCH_CONFIG
...
async def process_downloads_async(df):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        logging.info(
            f"Starting {len(tasks)} downloads with concurrency={CONFIG['CONCURRENT_DOWNLOADS']}..."
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Download decision PDFs from Climate Case Chart")
    ...
    # Apply test-run sampling (after trial batch filter)
    df = sample_dataframe(df, args.test_run, args.seed)
