...
Version: 2.0 (Config + Better JSON)
Date: November 2025
"""
...
from config import CONFIG, DB_CONFIG, DATABASE_FILE, UUID_NAMESPACE, LOGS_DIR, TEST_CONFIG

# Import database models
# Note: We insert scripts/phase0 to path to import models
...
def create_metadata_json(row, metadata_type='case'):
    ...
    IMPROVED: Splits list-like strings into actual JSON arrays.
    """
    metadata = {}
    ...
    if metadata_type == 'case':
        ...
        list_mappings = {
            ...
            'timeline_dates': 'Full timeline of events (dates)', # Keep as list of strings
...
def populate_database():
    ...
    try:
        ...
        if TEST_CONFIG['ENABLED']:
            ...
            df = df.head(limit)
            logging.info(f"⚠️ TEST MODE: Processing first {limit} rows (Configured in config.py)")
